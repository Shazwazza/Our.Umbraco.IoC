...
$SolutionPath = Join-Path -Path $SolutionRoot -ChildPath "Our.Umbraco.IoC.sln"
...
$include = @('Our.Umbraco.Ioc*.dll','Our.Umbraco.Ioc*.pdb')
foreach($project in $root.ChildNodes) {
	#$projectRelease = Join-Path -Path $ReleaseFolder -ChildPath "$($project.id)";
	#New-Item $projectRelease -Type directory

	$projectRoot = Join-Path -Path $SolutionRoot -ChildPath "$($project.id)";

	#$projectBin = Join-Path -Path $projectRoot -ChildPath "bin\Release";
	#Copy-Item "$projectBin\*.*" -Destination $projectRelease -Include $include
		
	$nuSpec = Join-Path -Path $projectRoot -ChildPath "$($project.id).nuspec";
		
	& $NuGet pack $nuSpec -OutputDirectory $ReleaseFolder -Version $project.version -Properties "copyright=$($Copyright);id=$($project.id);title=$($project.id)"
}
